...
export const legal = {
  ...
  '/privacidad/': () => legalPage('/privacidad/', 'Política de privacidad', `Cómo trata ${site.name} tus datos y tus archivos: la mayoría de herramientas funcionan en tu navegador sin enviar nada.`, html`
    <h2>Resumen</h2><ul><li>La mayoría de herramientas funcionan en tu navegador: <strong>tus archivos y textos no se envían a ningún servidor</strong>.</li><li>${site.gaId ? 'Solo usamos cookies de analítica (Google Analytics) si las aceptas en el aviso.' : 'No usamos cookies de seguimiento ni analítica de terceros.'}</li><li>Solo tratamos datos personales si nos escribes por el formulario de contacto.</li></ul>
    ...
    ${site.gaId ? html`<p><strong>Google Analytics:</strong> solo si aceptas las cookies de analítica, Google (Google Ireland Ltd.) recoge datos de navegación con la IP anonimizada. Base jurídica: tu consentimiento (art. 6.1.a RGPD), que puedes retirar en cualquier momento desde «Configurar cookies». Google puede transferir datos a EE. UU. al amparo del Marco de Privacidad de Datos UE-EE. UU. Consulta la <a href="/cookies/">política de cookies</a>.</p>` : ''}
  ...
  '/cookies/': () => legalPage('/cookies/', 'Política de cookies', `${site.name} no usa cookies de seguimiento ni publicidad. Qué almacenamiento local utilizamos y para qué.`, html`
    ${site.gaId ? html`<h2>Cookies de analítica (solo con tu consentimiento)</h2><p>Si pulsas «Aceptar» en el aviso de cookies, se carga Google Analytics 4 (Google Ireland Ltd.) para medir de forma agregada qué páginas y herramientas se usan. Si rechazas o no respondes, no se instala ninguna cookie de analítica. Nunca se envían tus archivos ni los textos que escribes.</p>
    <div class="table-wrap"><table class="table"><thead><tr><th scope="col">Cookie</th><th scope="col">Proveedor</th><th scope="col">Finalidad</th><th scope="col">Duración</th></tr></thead><tbody><tr><td>_ga</td><td>Google</td><td>Distinguir visitantes de forma anónima</td><td>2 años</td></tr><tr><td>_ga_&lt;ID&gt;</td><td>Google</td><td>Mantener el estado de la sesión</td><td>2 años</td></tr></tbody></table></div>
    <p>Puedes cambiar tu decisión cuando quieras: <button type="button" class="linklike" data-consent-reset>Configurar cookies</button>. También puedes borrar las cookies desde tu navegador.</p>` : html`<h2>¿Usamos cookies?</h2><p>No. ${site.name} no instala cookies propias ni de terceros con fines analíticos, publicitarios o de seguimiento. Por eso no te mostramos un banner de consentimiento.</p>`}
    <h2>Almacenamiento local técnico</h2><p>Guardamos en el <em>localStorage</em> de tu navegador, solo si usas esas funciones:</p>
    <div class="table-wrap"><table class="table"><thead><tr><th scope="col">Clave</th><th scope="col">Finalidad</th><th scope="col">Duración</th></tr></thead><tbody><tr><td>u.fav</td><td>Tus herramientas favoritas</td><td>Hasta que la borres</td></tr><tr><td>u.recent</td><td>Últimas herramientas usadas (solo su nombre, nunca tus archivos ni textos)</td><td>Hasta que la borres</td></tr>${site.gaId ? html`<tr><td>u.consent</td><td>Recordar si aceptaste o rechazaste las cookies de analítica</td><td>Hasta que la borres</td></tr>` : ''}</tbody></table></div>
